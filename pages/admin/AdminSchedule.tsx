...
const AdminSchedule = () => {
    ...
    const handleSaveItem = async (dayId: string, item: Partial<ScheduleItem>) => {
        ...
        // Trigger reverse sync to update corresponding groups
        await triggerReverseSync(day.day);
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-russo text-white mb-2 uppercase tracking-wider">Общее Расписание</h1>
                    <p className="text-white/50 font-manrope">Управление основным расписанием для главной страницы. <span className="text-sparta-gold/70 font-bold block mt-1">Внимание: Расписание конкретных тренировочных групп редактируется в разделе "Группы".</span></p>
                </div>
                <div className="flex gap-4">
                    ...
                    <Button
                        ...
                        variant="outline"
                        className="border-sparta-gold/30 text-sparta-gold flex items-center gap-2"
                    >
                        <RefreshCcw size={16} className={loading ? "animate-spin" : ""} />
                        Синхронизировать с Группами
                    </Button>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {days.filter(day => day.items && day.items.length > 0).map((day) => (
                    <motion.div
                        key={day.id}
                            ...
                            {day.items.map((item) => (
                                ...
                                >
                                    <div className="flex items-center gap-3 w-48 shrink-0">
                                        ...
                                        <span className="text-white font-russo tracking-widest text-sm">
                        ...
                        <form onSubmit={(e) => {
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-white/50 ml-2">Начало</label>
                                    <input
                                        name="time"
                                        type="time"
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-white/50 ml-2">Конец</label>
                                    <input
                                        name="endTime"
                                        type="time"
                                        defaultValue={editingItem?.item.endTime}
                                        className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-sparta-gold transition-colors"
                                    />
                                </div>
                            </div>
