...
import { ExcelImportModal, ImportRow } from '../../components/admin/ExcelImportModal';
...
const getCoachPhoto = (coach: any): string | null => {
    ...
    if (name.includes('Лебедева')) return '/aksinya-lebedeva.png';
    return null;
};
...
const AdminGroups = () => {
    ...
    const handleOpenEditor = (group?: Group) => {
        if (group) {
            ...
            setFormData({
                ...
                schedule: (group.schedule || []).map((s: any) => ({ day: s.day || '', time: s.time || '', activity: s.activity || '' }))
            });
        } else {
            setEditingId(null);
            setFormData({
    ...
    const handleConfirmImport = async (rowsToImport: ImportRow[]) => {
        ...
        try {
            ...
            for (const row of rowsToImport) {
                ...
                const age = parseInt(String(row.age) || '0', 10);
                ...
                groupsMap[groupName].students.push({ childName, age, phone, coachName, raw: row });
            ...
            for (const [groupName, data] of Object.entries(groupsMap)) {
                ...
                for (const student of data.students) {
                    const existingUser = users.find(u =>
                        ...
                        (student.phone && u.parentPhone === student.phone)
                    );

                    if (existingUser) {
                        if (targetGroupId && existingUser.groupId !== targetGroupId) {
                            batch.update(doc(db, "users", existingUser.id), { groupId: targetGroupId });
                        }
                    } else {
                        console.log('Parsed Excel Row:', student);
                        ...
                        let existingPendingDoc = pendingSnap.docs.find(d => {
                            const data = d.data();
                            return (cleanPhone && data.parentPhone === cleanPhone) || (targetGroupId && data.groupId === targetGroupId);
                        });

                        if (existingPendingDoc) {
                            batch.update(doc(db, "pending_students", existingPendingDoc.id), {
            ...
            await batch.commit();
            setSuccessMessage(`Импорт завершен! Групп создано: ${newGroupsCount}, Импортировано учеников: ${studentsProcessed}`);
                            ...
                            {selectedGroupIds.length > 0 ? (
                                ...
                                <>
                                    <button
                                        ...
                                        className="bg-green-600/20 text-green-400 border border-green-600/30 hover:bg-green-600/30 font-bold py-3 px-6 rounded-xl transition-all flex items-center gap-2"
                                    >
                                        <Upload size={20} />
                                        Импорт из файла
                                    </button>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3 overflow-hidden max-w-[80%]">
                                            <div
                                                ...
                                                className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all cursor-pointer ${selectedGroupIds.includes(group.id)
                                            ...
                                            <div className="overflow-hidden">
                                                <h3 className="text-xl font-bold text-white group-hover:text-sparta-gold transition-colors font-russo truncate">{group.name}</h3>
                                        ...
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleOpenEditor(group); }}
                                            className="bg-white/5 hover:bg-sparta-gold/20 hover:text-sparta-gold text-white/40 p-2 rounded-xl border border-white/10 shrink-0 transition-all"
                                    ...
                                    <div className="mb-4 bg-white/5 p-3 rounded-2xl border border-white/5">
                                        <div className="flex justify-between items-center text-xs mb-1.5 font-bold">
                                            <span className="text-white/60 flex items-center gap-1.5">
                                                <Users size={14} className="text-sparta-gold" /> Заполняемость
                                            </span>
                                            <span className="text-white">
                                                <span className="text-sparta-gold font-mono">{groupUsers.length}</span>
                                                <span className="text-white/40 font-mono"> / {group.maxStudents || 20}</span>
                                            </span>
                                        </div>
                                        <div className="w-full bg-white/10 h-2 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full rounded-full transition-all duration-500 ${
                                                    groupUsers.length >= (group.maxStudents || 20)
                                                        ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]'
                                                        : groupUsers.length >= (group.maxStudents || 20) * 0.8
                                                        ? 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.5)]'
                                                        : 'bg-sparta-gold shadow-[0_0_8px_rgba(212,175,55,0.4)]'
                                                }`}
                                                style={{ width: `${Math.min(100, Math.round((groupUsers.length / (group.maxStudents || 20)) * 100))}%` }}
                                            />
                                        </div>
                                        {pendingStudents.length > 0 && (
                                            <div className="mt-2 flex items-center gap-1 text-[11px] text-yellow-500/90 font-semibold">
                                                <Calendar size={12} /> {pendingStudents.length} ожид. регистрацию
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {(() => {
                                        ...
                                        const coachRole = coach?.role || "Главный тренер";
                                        const initials = coachName !== "Не назначен" ? coachName.split(' ').map((n: string) => n[0]).join('').toUpperCase() : '?';
                                            ...
                                            <div className="flex items-center gap-3 mb-4 bg-white/5 p-3 rounded-2xl border border-white/5">
                                                <div className="relative shrink-0">
                                                    {coachPhoto ? (
                                                        ...
                                                        <div className="w-10 h-10 rounded-full bg-sparta-gold/10 border border-amber-500/30 flex items-center justify-center text-sparta-gold text-xs font-bold font-russo shadow-md">
                                                            {initials}
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="overflow-hidden">
                                                    <p className="text-white text-sm font-bold truncate">{coachName}</p>
                                                    <p className="text-white/40 text-xs truncate">{coachRole}</p>
                                                </div>
                                            </div>
                                        );
                                    })()}
                                    ...
                                    <div className="mt-auto pt-4 border-t border-white/5 flex justify-between items-center">
                                        <span className="text-[10px] text-white/30 font-bold uppercase tracking-wider">Статус</span>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleOpenStatusModal(group); }}
                                            className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all flex items-center gap-1.5 hover:scale-105 ${
                                        ...
                                        >
                                            {group.currentStatus === 'cancelled' ? (
                                                <><AlertTriangle size={13} /> Отмена</>
                                            ) : group.currentStatus === 'substitute' ? (
                                                <><RefreshCw size={13} /> Замена</>
                                            ) : (
                                                <><Check size={13} /> Активна</>
                                            )}
                                            <span className="text-[10px] text-white/40 font-normal ml-0.5">• Изменить</span>
                                        </button>
                                    </div>
                                </div>
                            );
