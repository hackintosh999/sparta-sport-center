...
const AdminPromos = () => {
    ...
    const [type, setType] = useState<'discount' | 'balance'>('discount');
                                ...
                                <div key={promo.id} className={`grid grid-cols-7 gap-4 p-4 items-center hover:bg-white/5 transition-all group ${!isActive ? 'opacity-40 grayscale' : ''}`}>
                                    ...
                                    <div>
                                        <div className="inline-flex flex-col gap-1 items-start">
                                            ...
                                            `}>
                                                ...
                                                {promo.type === 'discount' ? 'Скидка' : 'Баланс'}
                                            </div>
                                    ...
                                    <div className="flex justify-end pr-2">
                                        <button
                                            ...
                                            className="p-2 bg-red-500/10 text-red-500/30 hover:bg-red-500 hover:text-white rounded-lg transition-all opacity-100 group-hover:opacity-100 z-30"
                                            title="Удалить код"
                                        >
                                            <Trash2 size={18} />
                                        </button>
                            ...
                            <form onSubmit={handleCreate} className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {[
                                        ...
                                        { id: 'balance', label: 'Баланс', icon: AlertCircle }
                                    ].map((t) => (
                                        <button
                                            key={t.id}
                                            type="button"
                                ...
                                <div className="grid grid-cols-2 gap-6 bg-black/20 p-6 rounded-2xl border border-white/5">
                                    ...
                                    <div>
                                        <label className="block text-white/50 text-xs font-bold uppercase tracking-wider mb-2">
                                            {type === 'discount' ? 'Размер скидки (%)' : 'Сумма зачисления'}
                                        </label>
                                    ...
                                    >
                                        {loading ? <Loader2 className="animate-spin" /> : (
                                            <div className="flex items-center gap-2">
                                                <Tag size={18} /> Создать Код{isBatch ? 'ы' : ''}
                                            </div>
                                        )}
                                    </button>
                                </div>
