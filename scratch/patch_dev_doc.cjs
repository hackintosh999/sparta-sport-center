...
async function run() {
    ...
    console.log('Patching Firestore document for:', targetEmail);
    const q = query(collection(db, 'users'), where('email', '==', targetEmail));
    const snap = await getDocs(q);

    if (snap.empty) {
        console.log('Creating document for', targetEmail);
        ...
        console.log('Created doc with role: super for', targetEmail);
    } else {
        for (const userDoc of snap.docs) {
            console.log(`Patching doc ${userDoc.id} for ${targetEmail}...`);
            ...
            console.log(`Successfully patched role: 'super' for doc ${userDoc.id}`);
        }
    }
    process.exit(0);
}
...
run().catch(err => {
    console.error('Error patching dev doc:', err);
    process.exit(1);
});
