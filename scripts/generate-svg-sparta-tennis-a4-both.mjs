...
function buildTennisA4Svg(cityName, addressLines) {
...
// 1. Novosibirsk Addresses (Планетная убрана)
const nskAddresses = [
  'ул. Большевистская 125,',
  'ул. Мясниковой 25/2'
];
const nskSvg = buildTennisA4Svg('г. Новосибирск', nskAddresses);

// 2. Chelyabinsk Addresses (Молодогвардейцев 7, Молдавская 14, 250-летия Челябинска 46)
const chelAddresses = [
  'ул. Молодогвардейцев 7,',
  'ул. Молдавская 14,',
  'ул. 250-летия Челябинска 46'
];
const chelSvg = buildTennisA4Svg('г. Челябинск', chelAddresses);
...
for (const p of nskTargets) {
  ...
  console.log('✅ [NSK A4] Saved SVG to: ' + p);
}
...
for (const p of chelTargets) {
  ...
  console.log('✅ [CHEL A4] Saved SVG to: ' + p);
}
...
console.log('✅ Updated A4 HTML files saved');

async function renderVisuals() {
  console.log('🚀 Launching Puppeteer to export high-res PNG & PDF for updated A4 variants...');
  ...
  console.log('🎉 [NSK A4] PNG and PDF exported successfully!');

  // Render Chelyabinsk A4
  ...
  console.log('🎉 [CHEL A4] PNG and PDF exported successfully!');

  await browser.close();
}

renderVisuals().then(() => {
  console.log('🌟 Updated Tennis A4 SVG, HTML, PNG, and PDF outputs generated perfectly!');
}).catch(err => {
