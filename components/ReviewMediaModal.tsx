...
import { X, ChevronLeft, ChevronRight, Star, Shield, Dumbbell, BadgeCheck, Code } from 'lucide-react';
                        ...
                        <div className="w-full md:w-[400px] border-l border-white/10 flex flex-col bg-[#0f0f0f]">
                            ...
                            <div className="p-6 border-b border-white/10 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="flex items-center gap-1 font-bold text-white text-lg">
                                            {review.userName}
                                            {review.userRole === 'admin' && <span title="Администратор"><BadgeCheck size={16} className="text-blue-500 shrink-0" /></span>}
                                            {review.userRole === 'trainer' && <span title="Тренер"><Dumbbell size={16} className="text-green-500 shrink-0" /></span>}
                                            {review.userRole === 'director' && <span title="Директор"><Star size={16} className="text-purple-500 shrink-0" /></span>}
                                            {review.userRole === 'developer' && <span title="Разработчик"><Code size={16} className="text-cyan-500 shrink-0" /></span>}
                                            {(review.userVerification as any)?.isVerified && <span title={(review.userVerification as any).title}><BadgeCheck size={16} className="text-blue-500 shrink-0" /></span>}
                                        </h4>
                                            ...
                                            {review.replies.map((reply) => (
                                                <div key={reply.id} className="bg-white/5 rounded-2xl p-4 border border-white/5">
                                                    <div className="flex items-center gap-3 mb-2">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-1 font-bold text-white text-sm leading-none">
                                                                {reply.userName}
                                                                {reply.userRole === 'admin' && <span title="Администратор"><BadgeCheck size={12} className="text-blue-500 shrink-0" /></span>}
                                                                {reply.userRole === 'trainer' && <span title="Тренер"><Dumbbell size={12} className="text-green-500 shrink-0" /></span>}
                                                                {reply.userRole === 'director' && <span title="Директор"><Star size={12} className="text-purple-500 shrink-0" /></span>}
                                                                {reply.userRole === 'developer' && <span title="Разработчик"><Code size={12} className="text-cyan-500 shrink-0" /></span>}
                                                                {(reply.userVerification as any)?.isVerified && <span title={(reply.userVerification as any).title}><BadgeCheck size={12} className="text-blue-500 shrink-0" /></span>}
                                                            </div>
