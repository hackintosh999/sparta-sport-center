...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const oneMinuteAgo = Date.now() - (60 * 1000); // 1 min buffer for heartbeats
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await addDoc(collection(db, 'messages'), {
                ...
                email,
                subject,
                category: QUICK_SUBJECTS.includes(subject) ? 'general' : 'custom',
                message,
                status: 'new',
                                ...
                                <p className="text-white/40 text-xs mt-1 flex items-center gap-2">
                                    {auth.currentUser && activeTicket
                                        ...
                                        : (
                                            <>
                                                {isSupportOnline ? (
                                                    <span className="flex items-center gap-1.5 text-green-500 font-bold transition-all duration-300">
                                                        <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)]" />
                                                        Поддержка онлайн
                                                    </span>
                                                ) : (
                                                    <span className="flex items-center gap-1.5 text-white/30 transition-all duration-300">
                                        ...
                                        <form onSubmit={handleSubmit} className="space-y-4">
                                                ...
                                                <div className="flex flex-wrap gap-2 mb-2">
                                                    {QUICK_SUBJECTS.map(qs => (
                                                        <button
                                                            ...
                                                            className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all border ${subject === qs
                                            ...
                                            <div className="pt-6 border-t border-white/5 mt-6">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    {SUPPORT_FAQ.slice(0, 4).map(faq => (
                                                        <div key={faq.id} className="p-3 bg-white/5 rounded-xl border border-white/5 hover:border-white/10 transition-all group">
                                                            <div className="text-[10px] font-bold text-sparta-gold mb-1">{faq.question}</div>
                                                            <div className="text-[9px] text-white/40 line-clamp-2 leading-relaxed">{faq.answer}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
