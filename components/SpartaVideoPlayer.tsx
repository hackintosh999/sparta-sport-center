...
const SpartaVideoPlayer: React.FC<SpartaVideoPlayerProps> = ({ src, poster, className = '', autoPlay = false }) => {
    ...
    // Check if it's a direct file or Supabase/Firebase URL
    const isNativeVideo = src?.includes('supabase.co') || src?.includes('firebasestorage') || src?.includes('firebase') || src?.match(/\.(mp4|webm|mov|ogg)$/i);
    ...
    useEffect(() => {
        if (isNativeVideo && nativePlayerRef.current) {
            if (isPlaying) {
                nativePlayerRef.current.play().catch(e => {
    ...
    useEffect(() => {
        if (isNativeVideo && nativePlayerRef.current) {
            ...
            nativePlayerRef.current.playbackRate = playbackRate;
            nativePlayerRef.current.loop = isLooping;
        }
    ...
    const skipForward = () => {
        if (isNativeVideo && nativePlayerRef.current) {
            nativePlayerRef.current.currentTime += 10;
        } else {
    ...
    const togglePip = () => {
        if (isNativeVideo && nativePlayerRef.current) {
            nativePlayerRef.current.requestPictureInPicture().catch(e => console.log('PiP not supported or failed', e));
            return;
        }
                ...
                <video
                    ...
                    onTimeUpdate={(e) => {
                        if (!showSettings) {
                            ...
                            setProgress((e.currentTarget.currentTime / (duration || 1)) * 100 || 0);
                        }
                    }}
