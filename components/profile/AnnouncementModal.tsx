...
import { Megaphone, X, ShieldAlert, Sparkles, Pin, Check, Flame, Trophy, AlertTriangle, Clock } from 'lucide-react';
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                <motion.div
                    ...
                    className="relative w-full max-w-lg bg-[#121214] border border-sparta-gold/30 rounded-3xl p-5 sm:p-6 shadow-2xl overflow-hidden"
                >
                    {/* Glowing Top Edge */}
                    <div className={`absolute top-0 left-0 right-0 h-1.5 ${priority === 'urgent' ? 'bg-gradient-to-r from-red-500 via-amber-500 to-red-500 animate-pulse' : 'bg-gradient-to-r from-amber-400 via-sparta-gold to-yellow-500'}`} />
                            ...
                            {PRESETS.map((p, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className="flex items-center gap-2 p-2.5 bg-white/5 hover:bg-sparta-gold/15 border border-white/10 hover:border-sparta-gold/40 rounded-xl text-left transition-all group"
                                    >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${priority === 'normal' ? 'bg-sparta-gold text-black shadow-lg shadow-sparta-gold/20' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'}`}
                                >
                                    <Sparkles size={13} />
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${priority === 'urgent' ? 'bg-red-500 text-white shadow-lg shadow-red-500/20' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'}`}
                                >
                                    <Flame size={13} />
